...
``` 
💡 list(map(int,input().split()))은 띄어쓰기로 구분된 문자열 요소들을 입력받아(input().split()), 정수로 바꾸어준 다음(map()), 리스트에 모두 저장하겠다는 의미이다.(list())
```
...
얼마 되지 않는 줄들을 입력받는 문제와는 달리, 반복문으로 여러 개의 테스트케이스를 받아서 문제를 해결해야할 때는 sys 모듈에 있는 sys.stdin.readline()를 사용해야 시간초과를 피할 수 있다.
